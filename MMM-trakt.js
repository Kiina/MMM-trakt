...
Module.register('MMM-trakt', {
  ...
  getDom: function () {
    ...
    if (Object.keys(this.traktData).length === 0 && this.traktCode === undefined) {
      ...
      for (const show in this.traktData) {
        const date = moment.utc(this.traktData[show].episode.first_aired).local()
        if (date.isBetween(moment(), moment().add(this.config.days - 1, 'd'), 'days', '[]')) {
          const tableRow = table.insertRow(-1)
          tableRow.className = 'normal'

          // Name
          const showTitleCell = tableRow.insertCell()
          ...
          const airtimeCell = tableRow.insertCell()
